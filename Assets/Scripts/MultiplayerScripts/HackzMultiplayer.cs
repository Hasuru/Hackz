...
public class HackzMultiplayer : NetworkBehaviour
{
    ...
    /**
     * Method for getting a certain player data given his character lobby index
     */
    public MP_PlayerData GetPlayerDataFromPlayerIndex(int playerIndex)
    {
    ...
    /**
     * Method for getting the player character from an index
     */
    ...
    private void ChangePlayerCharacterServerRpc(int characterId, ServerRpcParams serverRpcParams = default)
    {
        ...
        // To update the playerData, and because MP_PlayerData is a struct, we first need its index in the NetworkList
        int playerDataIndex = GetPlayerDataIndexFromClientId(serverRpcParams.Receive.SenderClientId);
        MP_PlayerData playerData = playerDataNetworkList[playerDataIndex];

        // Update the associated characterId
        playerData.characterId = characterId;

        // "Save" the changes in the NetworkList
        playerDataNetworkList[playerDataIndex] = playerData;
    }
